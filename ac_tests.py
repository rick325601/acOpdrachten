...
# Download ac_tests_supplemental and overwrite if proper access token stored in system variables.
ac_download.download_if_token_in_env(
    project_id=18212298,
    file_path="ac_tests_supplemental_implemented.py",
    output_name="ac_tests_supplemental.py"
)

import ac_tests_supplemental


# pylint: disable=W0613,R0201,C0103,R0913
# W0613 Unused argument: false positives due to defining classes inside functions
...
# R0913 Too many arguments: all defined functions and objects need to be passed to tests


# Global test suite instance, only used when running in non-ipython environments.
suite = tst.TestSuite()


def _detect_ipython() -> bool:
    """Determine whether current Python interpreter is part of ipython environment.

    Returns:
        whether current interpreter is operating from ipython environment."""
    try:
        _ = get_ipython()
        return True
    except NameError:
...
def run_tests(*tests):
    """Run one or more testcases correctly, regardless of whether interpreter is running from ipython environment
    or not.

    If interpreter is running from ipython environment, test suite will be created and ran with relevant tests as
    TextTestRunner so that test successes will be displayed immediately below code cell from which tests were executed.
    If not, the test cases will be added to global test suite 'suite' for test discovery from python , and not
    run immediately.

    Args:
        tests: one or more testcases."""
    ipython = _detect_ipython()
    testsuite = tst.TestSuite() if ipython else suite  # Create non-global test-suite if in ipython

    for test in tests:
        test_loader = tst.TestLoader().loadTestsFromTestCase(test)
        testsuite.addTest(test_loader)

    if ipython:  # Run test-suite if in ipython and display immediately.
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_vector_addition(vector_addition)
    run_tests(TestVectorAddition, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_negative_of_vector(negative_of_vector, vector_addition)
    run_tests(TestNegativeOfVector, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_scalar_product(scalar_product, vector_addition)
    run_tests(TestScalarProduct, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_inner_product(inner_product, scalar_product)
    run_tests(TestInnerProduct, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_vector_matrix_product(matrix_product)
    run_tests(TestVectorMatrixProduct, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_matrix_product(matrix_product)
    run_tests(TestMatrixProduct, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_neural_network(read_network, run_network, matrix_product)
    run_tests(TestNeuralNetwork, supplemental_tests)
...
"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_determinant(determinant_2, determinant_3, determinant)
    run_tests(TestDeterminant, supplemental_tests)
...
"""))
    supplemental_tests = ac_tests_supplemental.test_inverse_2(inverse_matrix_2)
    run_tests(TestInverse2, supplemental_tests)
...
| `test_inverse_no_inverse` | Correcte error voor een niet-omkeerbare vierkante matrix |"""))
    supplemental_tests = ac_tests_supplemental.test_inverse_(inverse_matrix, transpose)
    run_tests(TestInverse, supplemental_tests)
...
\begin{array}{|c|c|c|}\hline   5 & 5 & 6.5\\\hline   7 & 5.5 & 4\\\hline   4.5 & 6 & 6\\\hline \end{array}$$"""))
    supplemental_tests = ac_tests_supplemental.test_magisch_vierkant(magisch_vierkant)
    run_tests(TestMagischVierkant, supplemental_tests)
...
| `test_jump_right` | Zoekt de limiet van rechts in de piecewise functie |"""))
    supplemental_tests = ac_tests_supplemental.test_limit(limit_left, limit_right, limit)
    run_tests(TestLimit, supplemental_tests)
...
| `test_succ` | Zoekt de afgeleide van `succ` op $x=2$ |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_numeric_derivative(get_derivative_at)
    run_tests(TestNumericDerivative, supplemental_tests)
...
| `test_max_1` | De maximale waarde van de berekende snelheden klopt voor voertuig 2 |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_verkeer_snelheden(get_data, bereken_deltas)
    run_tests(TestVerkeerSnelheden, supplemental_tests)
...
| `test_primes` | Checkt of de variabele `primes` opgehoogd is |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_polynomial_derivative(get_derivative)
    run_tests(TestNumericDerivative, supplemental_tests)
...
def test_matrix_derivative(deriv_matrix):
    ...
    supplemental_tests = ac_tests_supplemental.test_matrix_derivative(deriv_matrix)
    run_tests(TestMatrixDerivative, supplemental_tests)
...
| `test_2x_plus_3` | Zoekt de afgeleide van $2x+3$, combineert alles |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_alfa(Constant,
                                                                                  Variable,
                                                                                  Sum,
                                                                                  Product,
                                                                                  Power)
    run_tests(TestSymbolicDifferentiationAlfa, supplemental_tests)
...
| `test_tan` | Zoekt de afgeleide van $\text{tan}(x)$ |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_bravo(Constant,
                                                                                   Variable,
                                                                                   Sum,
                                                                                   Product,
                                                                                   Power,
                                                                                   ...
                                                                                   Cos,
                                                                                   Tan)
    run_tests(TestSymbolicDifferentiationBravo, supplemental_tests)
...
| `test_ln` | Zoekt de afgeleide van $\text{ln}(x)$ |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_charlie(Constant,
                                                                                     Variable,
                                                                                     Sum,
                                                                                     Product,
                                                                                     Power,
                                                                                     ...
                                                                                     Ln,
                                                                                     Log)
    run_tests(TestSymbolicDifferentiationCharlie, supplemental_tests)
...
def test_symbolic_differentiation_charlie_eq(Constant, Variable, Sum, Product, Power, Sin, Cos, Tan, E, Exponent, Ln, Log):
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_charlie_eq(Constant,
                                                                                        Variable,
                                                                                        Sum,
                                                                                        Product,
                                                                                        Power,
                                                                                        ...
                                                                                        Ln,
                                                                                        Log)
    run_tests(TestSymbolicDifferentiationCharlieEq, supplemental_tests)
...
$$f^{}(x) =\text{log}_{5}(e^{x}) \mapsto f^{\prime} =\frac{1}{\text{ln}(5)}$$"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_delta(Constant,
                                                                                   Variable,
                                                                                   Sum,
                                                                                   Product,
                                                                                   Power,
                                                                                   ...
                                                                                   Ln,
                                                                                   Log)
    run_tests(TestSymbolicDifferentiationDelta, supplemental_tests)
...
def test_symbolic_differentiation_delta_eq(Constant, Variable, Sum, Product, Power, Sin, Cos, Tan, E, Exponent, Ln, Log):
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_delta_eq(Constant,
                                                                                      Variable,
                                                                                      Sum,
                                                                                      Product,
                                                                                      Power,
                                                                                      ...
                                                                                      Ln,
                                                                                      Log)
    run_tests(TestSymbolicDifferentiationDeltaEq, supplemental_tests)
...
def test_regressie(train, gradient, cost, data):
    ...
    supplemental_tests = ac_tests_supplemental.test_regressie(train, gradient, cost, data)
    run_tests(TestRegressie, supplemental_tests)
...
| `test_car_b_pos` | Controlleert de positie van de tweede auto |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_verkeer_posities(get_data, bereken_posities, vind_botsing)
    run_tests(TestVerkeerPosities, supplemental_tests)
...
| `test_bizarre` | Zoekt de integraal van `bizarre` voor de middelste heuvel, dit zou $1.86$ moeten benaderen |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_numeric_integral(get_integral_between)
    run_tests(TestNumericIntegral, supplemental_tests)
...
| `test_root` | Zoekt de integraal van `x_root` ($\sqrt x$) |"""))
    ...
    supplemental_tests = ac_tests_supplemental.test_polynomial_integral(get_integral)
    run_tests(TestNumericIntegral, supplemental_tests)
...
def test_symbolic_integration_alfa(Constant, Variable, Sum, Product, Power):
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_integration_alfa(Constant, Variable, Sum, Product, Power)
    run_tests(TestSymbolicIntegrationAlfa, supplemental_tests)
...
def test_symbolic_integration_alfa_eq(Constant, Variable, Sum, Product, Power):
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_integration_alfa_eq(Constant, Variable, Sum, Product, Power)
    run_tests(TestSymbolicIntegrationAlfaEq, supplemental_tests)
...
def test_symbolic_integration_bravo(Constant, Variable, Sum, Product, Power, Sin, Cos, Tan, E, Exponent, Ln, Log):
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_differentiation_bravo(Constant,
                                                                                   ...
                                                                                   Ln,
                                                                                   Log)
    run_tests(TestSymbolicIntegrationBravo, supplemental_tests)
...
def test_symbolic_integration_bravo_eq(Constant, Variable, Sum, Product, Power, Sin, Cos, Tan, E, Exponent, Ln, Log):
    ...
    supplemental_tests = ac_tests_supplemental.test_symbolic_integration_bravo_eq(Constant,
                                                                                  Variable,
                                                                                  Sum,
                                                                                  Product,
                                                                                  Power,
                                                                                  ...
                                                                                  Ln,
                                                                                  Log)
    run_tests(TestSymbolicIntegrationBravoEq, supplemental_tests)
...
def test_euler(afgeleide_functie, euler):
    ...
    supplemental_tests = ac_tests_supplemental.test_euler(afgeleide_functie, euler)
    run_tests(TestEuler, supplemental_tests)
